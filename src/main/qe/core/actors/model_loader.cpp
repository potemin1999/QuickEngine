...
void ModelLoader::load_object(Object *container, const char *path, const char *name) {
    ModelLoader::load_object(container, (char *) path, (char *) name);
}

void ModelLoader::load_object(Object *container, char *path, char *name) {
	...
	while (!file.eof()) {
        ...
        if (line.find('#') == 0) {
			...
			*m.material = *last_mat;// &materials[materials.size() - 1];
			...
			meshes.push_back(m);
			log("    creating mesh group %s, uses material %s\n", m.name , (last_mat!=nullptr ? last_mat->name : "nullptr") );
            //delete mesh_n;
            //delete mesh_name;
			...
			for (int i = 1; i < 4; i++) {
                ...
                norm &n = normales[atoi(items[2].c_str()) - 1];
	...
	container->mesh_count = (unsigned long) meshes.size();
