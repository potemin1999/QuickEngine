...
void Camera::update_perspective_matrix() {
	...
	printf("perspective matrix updated aspect %f  fovy %f  distance %f\n", aspect, fovy, farZ);
}
...
void Camera::position_camera(vec3* pos, vec3* view, vec3* up) {
	...
	mUp.z = up->z;
	normalize(&mView);
	printf("set  position x:%f y:%f z:%f\n", mPos.x, mPos.y, mPos.z);
	printf("set   look at x:%f y:%f z:%f\n", mView.x,mView.y,mView.z);
	printf("set up vector x:%f y:%f z:%f\n", mUp.x,mUp.y,mUp.z);
}
