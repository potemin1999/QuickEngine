...
//      such that Transpose<SelfAdjointView<.,.> > is valid. (currently TriangularBase::transpose() is overloaded to make it work)
    ...
    struct Assignment<DstXprType, SrcXprType, Functor, SparseSelfAdjoint2Sparse> {
        ...
        static void run(DynamicSparseMatrix <DestScalar, ColMajor, StorageIndex> &dst, const SrcXprType &src,
                        const AssignOpType &/*func*/) {
...
namespace internal {
    ...
    inline void
    sparse_selfadjoint_time_dense_product(const SparseLhsType &lhs, const DenseRhsType &rhs, DenseResType &res,
        ...
        for (Index k = 0; k < rhs.cols(); ++k) {
            for (Index j = 0; j < lhs.outerSize(); ++j) {
                ...
                typename ScalarBinaryOpTraits<AlphaType, typename DenseRhsType::Scalar>::ReturnType rhs_j(
                        alpha * rhs(j, k));
    ...
    struct generic_product_impl<LhsView, Rhs, SparseSelfAdjointShape, DenseShape, ProductType>
        ...
        template<typename Dest>
        static void
        scaleAndAddTo(Dest &dst, const LhsView &lhsView, const Rhs &rhs, const typename Dest::Scalar &alpha) {
    ...
    struct generic_product_impl<Lhs, RhsView, DenseShape, SparseSelfAdjointShape, ProductType>
        ...
        template<typename Dest>
        static void
        scaleAndAddTo(Dest &dst, const Lhs &lhs, const RhsView &rhsView, const typename Dest::Scalar &alpha) {
            ...
            internal::sparse_selfadjoint_time_dense_product<RhsView::Mode>(rhsNested.transpose(), lhsNested.transpose(),
                                                                           dstT, alpha);
        }
    };
...
// TODO: maybe the copy could be handled by generic_product_impl so that these overloads would not be needed anymore
    ...
    product_evaluator(const XprType &xpr)
        ...
        generic_product_impl<typename Rhs::PlainObject, Rhs, SparseShape, SparseShape, ProductTag>::evalTo(m_result,
                                                                                                           m_lhs,
                                                                                                           xpr.rhs());
    }
...
product_evaluator(const XprType &xpr)
    ...
    ::new(static_cast<Base *>(this)) Base(m_result);
    generic_product_impl<Lhs, typename Lhs::PlainObject, SparseShape, SparseShape, ProductTag>::evalTo(m_result,
                                                                                                       xpr.lhs(),
                                                                                                       m_rhs);
}
