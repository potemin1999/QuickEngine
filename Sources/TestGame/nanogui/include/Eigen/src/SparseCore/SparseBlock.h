...
// Subset of columns or rows
    ...
    inline BlockImpl(XprType
    & xpr,
    Index startRow, Index
    startCol,
    Index blockRows, Index
    blockCols)
    :
    ...
    Index nonZeros() const {
        ...
        nnz = 0;
        Index
        end = m_outerStart + m_outerSize.value();
    ...
    inline const Scalar coeff(Index
    row,
    Index col
    ) const {
    return m_matrix.
    coeff(row
    + (IsRowMajor ? m_outerStart : 0), col + (IsRowMajor ? 0 :  m_outerStart));
}
...
namespace internal {
    ...
    inline BlockType &operator=(const SparseMatrixBase <OtherDerived> &other) {
        ...
        Ref<
        const SparseMatrix<Scalar, IsRowMajor ? RowMajor : ColMajor, StorageIndex> > tmp(other.derived());
        ...
        nnz = tmp.nonZeros();
        Index
        start = m_outerStart == 0 ? 0
                                  : m_matrix.outerIndexPtr()[m_outerStart]; // starting position of the current block
        Index
        end = m_matrix.outerIndexPtr()[m_outerStart + m_outerSize.value()]; // ending position of the current block
        Index
        block_size = end - start;                                                // available room in the current block
        Index
        ...
        if (nnz > free_size) {
            ...
            internal::smart_copy(tmp.valuePtr() + tmp_start, tmp.valuePtr() + tmp_start + nnz,
...
const Scalar &lastCoeff() const {
    ...
    if (m_matrix.isCompressed())
        ...
        return m_matrix.valuePtr()[m_matrix.outerIndexPtr()[m_outerStart] + m_matrix.innerNonZeroPtr()[m_outerStart] -
                                   1];
}
...
class BlockImpl<SparseMatrix < _Scalar, _Options, _StorageIndex>
...
inline BlockImpl(SparseMatrixType
& xpr,
Index startRow, Index
startCol,
Index blockRows, Index
...
friend struct internal::unary_evaluator<
        Block < XprType, BlockRows, BlockCols, InnerPanel>, internal::IteratorBased, Scalar >;
...
{
enum {
    IsRowMajor = unary_evaluator::IsRowMajor
};
const XprType &m_block;
Index m_end;
public:
...
:
EvalIterator(aEval
.m_argImpl, outer + (IsRowMajor ? aEval.m_block.
...
m_end(IsRowMajor ? aEval.m_block.startCol() + aEval.m_block.blockCols() : aEval.m_block.startRow() +
...
struct unary_evaluator<
        Block < SparseMatrix < _Scalar, _Options, _StorageIndex>, BlockRows, BlockCols, true>, IteratorBased>
: evaluator<SparseCompressedBase<
        Block < SparseMatrix < _Scalar, _Options, _StorageIndex>, BlockRows, BlockCols, true> > >
{
...
struct unary_evaluator<Block<const SparseMatrix <_Scalar, _Options, _StorageIndex>, BlockRows, BlockCols, true>, IteratorBased>
        : evaluator<SparseCompressedBase <
                    Block<const SparseMatrix <_Scalar, _Options, _StorageIndex>, BlockRows, BlockCols, true> > >
{
