...
NAMESPACE_BEGIN(nanogui)
    ...
    void TabHeader::drawControls(NVGcontext *ctx) {
        ...
        Vector2f leftIconPos = mPos.cast<float>() +
                               Vector2f(xScaleLeft * theme()->mTabControlWidth, yScaleLeft * mSize.cast<float>().y());
        ...
        Vector2f rightIconPos =
                mPos.cast<float>() + Vector2f(mSize.cast<float>().x(), mSize.cast<float>().y() * yScaleRight) -
    ...
    TabHeader::ClickLocation TabHeader::locateClick(const Vector2i &p) {
        ...
        bool hitLeft = (leftDistance >= 0).all() &&
                       (leftDistance < Vector2i(theme()->mTabControlWidth, mSize.y()).array()).all();
        if (hitLeft)
        ...
        bool hitRight = (rightDistance >= 0).all() &&
                        (rightDistance < Vector2i(theme()->mTabControlWidth, mSize.y()).array()).all();
        if (hitRight)
