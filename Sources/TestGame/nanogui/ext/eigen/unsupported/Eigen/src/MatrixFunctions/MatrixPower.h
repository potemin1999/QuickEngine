 ...
 */
    ...
    public:
        ...
        inline void evalTo(ResultType &res) const { m_pow.compute(res, m_p); }
    ...
    void MatrixPowerAtomic<MatrixType>::computePade(int degree, const MatrixType &IminusT, ResultType &res) const {
        ...
        for (--i; i; --i) {
            res = (MatrixType::Identity(IminusT.rows(), IminusT.cols()) + res).template triangularView<Upper>()
                    .solve((i == 1 ? -m_p : i & 1 ? (-m_p - i / 2) / (2 * i) : (m_p - i / 2) / (2 * i - 2)) *
                           IminusT).eval();
        }
...
// This function assumes that res has the correct size (see bug 614)
    ...
    void MatrixPowerAtomic<MatrixType>::computeBig(ResultType &res) const {
        ...
        const RealScalar maxNormForPade = digits <= 24 ? 4.3386528e-1L                            // single precision
                                                       : digits <= 53
                                                         ? 2.789358995219730e-1L                    // double precision
                                                         : digits <= 64
                                                           ? 2.4471944416607995472e-1L                // extended precision
                                                           : digits <= 106
                                                             ? 1.1016843812851143391275867258512e-1L    // double-double
    ...
    inline int MatrixPowerAtomic<MatrixType>::getPadeDegree(long double normIminusT) {
        ...
        const long double maxNormForPade[] = {6.3854693117491799460e-3L /* degree = 3 */ , 2.6394893435456973676e-2L,
                                              6.4216043030404063729e-2L, 1.1701165502926694307e-1L,
                                              1.7904284231268670284e-1L, 2.4471944416607995472e-1L};
    ...
    MatrixPowerAtomic<MatrixType>::computeSuperDiag(const ComplexScalar &curr, const ComplexScalar &prev,
                                                    RealScalar p) {
        using std::ceil;
        using std::exp;
        using std::log;
        ...
        ComplexScalar w =
                numext::log1p((curr - prev) / prev) / RealScalar(2) + ComplexScalar(0, EIGEN_PI * unwindingNumber);
 ...
 */
    ...
    public:
         ...
         * constructor.
         */
        const MatrixPowerParenthesesReturnValue<MatrixType>
        operator()(RealScalar p) { return MatrixPowerParenthesesReturnValue<MatrixType>(*this, p); }
 ...
 */
    ...
    public:
         ...
         */
        template<typename ResultType>
        inline void evalTo(ResultType &res) const { MatrixPower<PlainObject>(m_A.eval()).compute(res, m_p); }
 ...
 */
    ...
    public:
         ...
         */
        template<typename ResultType>
        inline void evalTo(ResultType &res) const { res = (m_p * m_A.log()).exp(); }
    ...
    MatrixBase<Derived>::pow(const RealScalar &p) const { return MatrixPowerReturnValue<Derived>(derived(), p); }
