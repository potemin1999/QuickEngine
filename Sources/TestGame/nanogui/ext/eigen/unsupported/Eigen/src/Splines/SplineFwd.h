...
namespace Eigen {
    ...
    struct SplineTraits<Spline<_Scalar, _Dim, _Degree>, Dynamic> {
        ...
        enum {
            OrderAtCompileTime =
            _Degree == Dynamic ? Dynamic : _Degree + 1 /*!< The spline curve's order at compile-time. */ };
        enum {
        ...
        enum {
            ...
            Dimension == 1 ? RowMajor : ColMajor /*!< The derivative type's memory layout. */ };
    ...
    struct SplineTraits<Spline<_Scalar, _Dim, _Degree>, _DerivativeOrder>
        ...
        enum {
            ...
            _Degree == Dynamic ? Dynamic : _Degree + 1 /*!< The spline curve's order at compile-time. */ };
        enum {
            NumOfDerivativesAtCompileTime = _DerivativeOrder == Dynamic ? Dynamic : _DerivativeOrder +
                                                                                    1 /*!< The number of derivatives defined for the current spline. */ };

        enum {
            DerivativeMemoryLayout = _Dim == 1 ? RowMajor : ColMajor /*!< The derivative type's memory layout. */ };
