...
// Proxy to view a regular vector as a block vector
    ...
    public:
        ...
        typedef Ref<const Matrix<typename BlockSparseMatrixT::Scalar, (RowsAtCompileTime == 1) ? 1
                                                                                               : BlockSize, (ColsAtCompileTime ==
                                                                                                             1) ? 1
                                                                                                                : BlockSize> > Scalar;
        typedef typename BlockSparseMatrixT::Index Index;
    public:
...
// Block version of the sparse dense product
    ...
    public:
        ...
        void scaleAndAddTo(Dest &dest, const typename Rhs::Scalar &alpha) const {
            ...
            internal::sparse_time_dense_product(BlockSparseMatrixView<Lhs>(m_lhs),
                                                BlockVectorView<Lhs, Rhs>(m_lhs, m_rhs), tmpDest, alpha);
        }
    ...
    public:
        ...
        typedef Matrix<RealScalar, _BlockAtCompileTime, _BlockAtCompileTime, IsColMajor ? ColMajor
                                                                                        : RowMajor> BlockRealScalar;
        typedef typename internal::conditional<
                _BlockAtCompileTime == Dynamic, Scalar, BlockScalar>::type BlockScalarReturnType;
    ...
    public:
                ...
                for (StorageIndex j = blockOuterIndex(bj); j < blockOuterIndex(bj + 1); ++j) {
                    ...
                    for (; it_spmat; ++it_spmat) {
                        ...
                        if (m_blockSize == Dynamic) {
                            ...
                            idxVal += (j - blockOuterIndex(bj)) * blockOuterSize(bj) + it_spmat.index() -
                                      m_innerOffset[bi];
                        } else {
                            // All blocks before
        ...
        void setBlockStructure(const MatrixType &blockPattern) {
            ...
            for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
                ...
                for (StorageIndex idx = 0; idx < nzBlockIdx.size(); ++idx) {
                    ...
                    if (m_blockSize == Dynamic)
                        m_blockPtr[offset] =
                                m_blockPtr[offset - 1] + blockInnerSize(nzBlockIdx[idx]) * blockOuterSize(bj);
        ...
        void setFromTriplets(const InputIterator &begin, const InputIterator &end) {
            ...
            for (InputIterator it(begin); it != end; ++it) {
                eigen_assert(it->row() >= 0 && it->row() < this->blockRows() && it->col() >= 0 &&
                             it->col() < this->blockCols());
