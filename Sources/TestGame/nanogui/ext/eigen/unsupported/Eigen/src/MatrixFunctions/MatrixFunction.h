...
/** \brief Compute size of each cluster given a partitioning */
        ...
        matrix_function_compute_cluster_size(const ListOfClusters &clusters, Matrix<Index, Dynamic, 1> &clusterSize) {
            ...
            for (typename ListOfClusters::const_iterator cluster = clusters.begin();
...
/** \brief Compute mapping of eigenvalue indices to cluster indices */
        ...
        void matrix_function_compute_map(const EivalsType &eivals, const ListOfClusters &clusters,
        ...
        void matrix_function_compute_above_diagonal(const MatrixType &T, const VectorType &blockStart,
            ...
            for (Index k = 1; k < clusterSize.rows(); k++) {
                for (Index i = 0; i < clusterSize.rows() - k; i++) {
                    ...
                    DynMatrixType B = -T.block(blockStart(i + k), blockStart(i + k), clusterSize(i + k),
                                               clusterSize(i + k));
