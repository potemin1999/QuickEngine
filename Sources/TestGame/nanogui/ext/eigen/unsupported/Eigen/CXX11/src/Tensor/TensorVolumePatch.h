...
template<DenseIndex Planes, DenseIndex Rows, DenseIndex Cols, typename XprType>
struct nested<TensorVolumePatchOp < Planes, Rows, Cols, XprType>, 1, typename eval<
...
public:
    ...
    TensorVolumePatchOp(const XprType &expr, DenseIndex patch_planes, DenseIndex patch_rows, DenseIndex patch_cols,
                        ...
                        DenseIndex plane_inflate_strides, DenseIndex row_inflate_strides,
            ...
            : m_xpr(expr), m_patch_planes(patch_planes), m_patch_rows(patch_rows), m_patch_cols(patch_cols),
              ...
              m_padding_explicit(false), m_padding_top_z(0), m_padding_bottom_z(0), m_padding_top(0),
              m_padding_bottom(0), m_padding_left(0), m_padding_right(0),
    ...
    TensorVolumePatchOp(const XprType &expr, DenseIndex patch_planes, DenseIndex patch_rows, DenseIndex patch_cols,
            ...
            : m_xpr(expr), m_patch_planes(patch_planes), m_patch_rows(patch_rows), m_patch_cols(patch_cols),
              ...
              m_plane_inflate_strides(plane_inflate_strides), m_row_inflate_strides(row_inflate_strides),
              m_col_inflate_strides(col_inflate_strides),
              m_padding_explicit(true), m_padding_top_z(padding_top_z), m_padding_bottom_z(padding_bottom_z),
              m_padding_top(padding_top), m_padding_bottom(padding_bottom),
    ...
    patch_planes() const { return m_patch_planes; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    patch_rows() const { return m_patch_rows; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    patch_cols() const { return m_patch_cols; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    plane_strides() const { return m_plane_strides; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    row_strides() const { return m_row_strides; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    col_strides() const { return m_col_strides; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    in_plane_strides() const { return m_in_plane_strides; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    in_row_strides() const { return m_in_row_strides; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    in_col_strides() const { return m_in_col_strides; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    plane_inflate_strides() const { return m_plane_inflate_strides; }
    ...
    col_inflate_strides() const { return m_col_inflate_strides; }
    ...
    padding_top_z() const { return m_padding_top_z; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    padding_bottom_z() const { return m_padding_bottom_z; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    padding_top() const { return m_padding_top; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    padding_bottom() const { return m_padding_bottom; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    padding_left() const { return m_padding_left; }

    EIGEN_DEVICE_FUNC
            DenseIndex

    padding_right() const { return m_padding_right; }

    EIGEN_DEVICE_FUNC
            PaddingType

    padding_type() const { return m_padding_type; }
...
struct TensorEvaluator<const TensorVolumePatchOp<Planes, Rows, Cols, ArgType>, Device> {
    ...
    TensorEvaluator(const XprType &op, const Device &device)
        ...
        if (op.padding_explicit()) {
            ...
            m_outputRows = numext::ceil(
                    (m_input_rows_eff + op.padding_top() + op.padding_bottom() - m_patch_rows_eff + 1.f) /
                    static_cast<float>(m_row_strides));
            m_outputCols = numext::ceil(
                    (m_input_cols_eff + op.padding_left() + op.padding_right() - m_patch_cols_eff + 1.f) /
            ...
            switch (op.padding_type()) {
                case PADDING_VALID:
                    m_outputPlanes = numext::ceil(
                            (m_input_planes_eff - m_patch_planes_eff + 1.f) / static_cast<float>(m_plane_strides));
                    m_outputRows = numext::ceil(
                            (m_input_rows_eff - m_patch_rows_eff + 1.f) / static_cast<float>(m_row_strides));
                    m_outputCols = numext::ceil(
                            (m_input_cols_eff - m_patch_cols_eff + 1.f) / static_cast<float>(m_col_strides));
                    m_planePaddingTop = 0;
                    m_rowPaddingTop = 0;
                    m_colPaddingLeft = 0;
                    break;
    ...
    CoeffReturnType coeff(Index index) const {
        ...
        const Index patch3DIndex = (NumDims == 5) ? patchIndex : (index - otherIndex * m_otherStride) /
                                                                 m_fastPatchStride;
        ...
        const Index origInputCol = (m_col_inflate_strides == 1) ? inputCol : ((inputCol >= 0) ? (inputCol /
        ...
        const Index origInputRow = (m_row_inflate_strides == 1) ? inputRow : ((inputRow >= 0) ? (inputRow /
        ...
        const Index origInputPlane = (m_plane_inflate_strides == 1) ? inputPlane : ((inputPlane >= 0) ? (inputPlane /
    ...
    PacketReturnType packet(Index index) const {
        ...
        const Index patch3DIndex = (NumDims == 5) ? patchIndex : (indices[0] - otherIndex * m_otherStride) /
                                                                 m_fastPatchStride;
