...
template<typename NewDimensions, typename XprType>
struct nested<TensorReshapingOp < NewDimensions, XprType>, 1, typename eval<
...
public:
    ...
    dimensions() const { return m_dims; }
    ...
    TensorReshapingOp &operator=(const TensorReshapingOp &other) {
    ...
    TensorReshapingOp &operator=(const OtherDerived &other) {
...
struct TensorEvaluator<const TensorReshapingOp<NewDimensions, ArgType>, Device> {
    ...
    *
    ...
    data() const { return const_cast<Scalar *>(m_impl.data()); }

    EIGEN_DEVICE_FUNC const TensorEvaluator<ArgType, Device>
    &
...
struct TensorEvaluator<TensorReshapingOp<NewDimensions, ArgType>, Device>
    ...
    CoeffReturnType &coeffRef(Index index) {
...
template<typename StartIndices, typename Sizes, typename XprType>
struct nested<TensorSlicingOp < StartIndices, Sizes, XprType>, 1, typename eval<
...
public:
    ...
    startIndices() const { return m_indices; }

    EIGEN_DEVICE_FUNC
    const Sizes
    &

    sizes() const { return m_sizes; }
    ...
    TensorSlicingOp &operator=(const OtherDerived &other) {
    ...
    TensorSlicingOp &operator=(const TensorSlicingOp &other) {
...
struct TensorEvaluator<const TensorSlicingOp<StartIndices, Sizes, ArgType>, Device> {
    ...
    TensorEvaluator(const XprType &op, const Device &device)
            : m_impl(op.expression(), device), m_device(device), m_dimensions(op.sizes()),
...
template<typename StartIndices, typename StopIndices, typename Strides, typename XprType>
struct nested<TensorStridingSlicingOp < StartIndices, StopIndices, Strides, XprType>, 1, typename eval<
...
public:
    ...
    startIndices() const { return m_startIndices; }

    EIGEN_DEVICE_FUNC
    const StartIndices
    &

    stopIndices() const { return m_stopIndices; }
    ...
    TensorStridingSlicingOp &operator=(const TensorStridingSlicingOp &other) {
    ...
    TensorStridingSlicingOp &operator=(const OtherDerived &other) {
...
struct TensorEvaluator<const TensorStridingSlicingOp<StartIndices, StopIndices, Strides, ArgType>, Device> {
    ...
    TensorEvaluator(const XprType &op, const Device &device)
            : m_impl(op.expression(), device), m_device(device), m_strides(op.strides()),
              m_exprStartIndices(op.startIndices()), m_exprStopIndices(op.stopIndices()) {
    ...
    const StartIndices &exprStartIndices() const { return m_exprStartIndices; }

    //use by sycl
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE

    const StartIndices &exprStopIndices() const { return m_exprStopIndices; }
