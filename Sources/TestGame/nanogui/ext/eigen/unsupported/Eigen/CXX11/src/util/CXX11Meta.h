 ...
 */
        ...
        struct gen_numeric_list_swapped_pair : gen_numeric_list_swapped_pair<T,
                n - 1, a, b, start, (start + n - 1) == a ? b : ((start + n - 1) == b ? a : (start + n - 1)), ii...> {
        };
...
/* list manipulation: extract slices */
        ...
        struct take<n, numeric_list<T, a, as...>> : concat<numeric_list<T, a>, typename take<
                n - 1, numeric_list<T, as...>>::type> {
        };
        ...
        struct h_skip {
            ...
            constexpr static inline typename h_skip_helper_numeric<T, n, ii...>::type
            helper(numeric_list<T, ii...>) { return typename h_skip_helper_numeric<T, n, ii...>::type(); }

            template<typename... tt>
            constexpr static inline typename h_skip_helper_type<n, tt...>::type
            helper(type_list<tt...>) { return typename h_skip_helper_type<n, tt...>::type(); }
        };
...
/* apply_op to list */
        ...
        struct h_apply_op {
            ...
            constexpr static typename h_apply_op_helper<from_left, op, additional_param, values...>::type
            helper(type_list<values...>) { return typename h_apply_op_helper<from_left, op, additional_param, values...>::type(); }
        };
...
/* see if an element is in a list and check for global flags */
        ...
        template<
                ...
                int last_check_flags
        >
        struct contained_in_list_gf<test, check_against, type_list<EIGEN_TPL_PP_SPEC_HACK_USE(
                empty)>, default_flags, false, last_check_flags> {
            constexpr static bool value = false;
...
/* generic reductions */
        ...
        struct reduce<Reducer, A, Ts...> {
            ...
            run(A a, Ts... ts) -> decltype(Reducer::run(a, reduce<Reducer, Ts...>::run(ts...))) {
...
/* generic binary operations */
        ...
        struct sum_op {
            template<typename A, typename B>
            EIGEN_DEVICE_FUNC constexpr static inline auto run(A a, B b)
        ...
        struct logical_and_op {
            template<typename A, typename B>
            constexpr static inline auto run(A a, B b) -> decltype(a && b) { return a && b; }
        };
...
/* generic unary operations */
        ...
        struct not_op {
            template<typename A>
            constexpr static inline auto run(A a) -> decltype(!a) { return !a; }
        };
        ...
        struct greater_equal_zero_op {
            template<typename A>
            constexpr static inline auto run(A a) -> decltype(a >= 0) { return a >= 0; }
        };
...
// an infinite loop)
        ...
        struct h_array_reduce<Reducer, T, N, 0> {
            EIGEN_DEVICE_FUNC constexpr static inline T

            run(const array<T, N> &arr, T) {
        ...
        struct h_array_reduce<Reducer, T, 0> {
            ...
            run(const array<T, 0> &, T identity) {
                return identity;
            }
        };
        ...
        return
        h_array_reduce<Reducer, T, N>::run(arr, identity
        );
    }
...
{
return
array_reduce<product_op, T, N>(arr,
static_cast<T>(1));
}
...
constexpr inline auto h_array_zip_and_reduce(array <A, N> a, array <B, N> b,
                                             numeric_list<int, n...>) -> decltype(reduce<Reducer, typename id_numeric<int, n, decltype(Op::run(
        A(), B()))>::type...>::run(Op::run(array_get<n>(a), array_get<n>(b))...)) {
...
array_zip_and_reduce(array <A, N> a, array <B, N> b) -> decltype(h_array_zip_and_reduce<Reducer, Op, A, B, N>(a, b,
...
constexpr inline auto h_array_apply_and_reduce(array <A, N> arr,
                                               numeric_list<int, n...>) -> decltype(reduce<Reducer, typename id_numeric<int, n, decltype(Op::run(
...
constexpr inline auto array_apply_and_reduce(array <A, N> a) -> decltype(h_array_apply_and_reduce<Reducer, Op, A, N>(a,
                                                                                                                     typename gen_numeric_list<int, N>::type())) {
