...
template<typename TargetType, typename XprType>
struct nested<TensorConversionOp < TargetType, XprType>, 1, typename eval<
...
template<bool SameType, typename Eval, typename Scalar>
...
template<typename Eval, typename Scalar>
...
struct TensorEvaluator<const TensorConversionOp<TargetType, ArgType>, Device> {
    ...
    bool evalSubExprsIfNeeded(Scalar *data) {
        return ConversionSubExprEval<internal::is_same<TargetType, SrcType>::value, TensorEvaluator<ArgType, Device>, Scalar>::run(
                m_impl, data);
    }
...
protected:
    ...
    struct PacketConv {
        ...
        PacketReturnType run(const TensorEvaluator<ArgType, Device> &impl, Index index) {
            internal::scalar_cast_op <SrcType, TargetType> converter;
            EIGEN_ALIGN_MAX
