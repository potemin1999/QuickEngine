...
// Boilerplate code
    namespace internal {
        ...
        template<std::size_t n, typename Dimension>
        struct dget {
        ...
        struct fixed_size_tensor_index_linearization_helper {
            ...
            static inline Index
            run(array<Index, NumIndices> const& indices,
        ...
        struct fixed_size_tensor_index_linearization_helper<Index, NumIndices, 0, RowMajor> {
            template<typename Dimensions> EIGEN_DEVICE_FUNC
            static inline Index
            run(array<Index, NumIndices> const&, const Dimensions&)
            {
                return 0;
            }
        };
...
// Fixed size
    ...
    struct Sizes {
        ...
        std::ptrdiff_t operator[](const std::size_t index) const {
...
// Dynamic size
    ...
    struct DSizes : array<DenseIndex, NumDims> {
        ...
        DenseIndex IndexOfColMajor(const array <DenseIndex, NumDims> &indices) const {
            return internal::tensor_index_linearization_helper<DenseIndex, NumDims, NumDims - 1, false>::run(indices,
        ...
        DenseIndex IndexOfRowMajor(const array <DenseIndex, NumDims> &indices) const {
            return internal::tensor_index_linearization_helper<DenseIndex, NumDims, NumDims - 1, true>::run(indices,
                                                                                                            *static_cast<const Base *>(this));
        }
    };
...
// Boilerplate
    namespace internal {
        ...
        struct tensor_vsize_index_linearization_helper {
            ...
            Index run(array <Index, NumIndices> const &indices, std::vector <DenseIndex> const &dimensions) {
                return array_get<RowMajor ? n : (NumIndices - n - 1)>(indices) +
                       array_get<RowMajor ? n : (NumIndices - n - 1)>(dimensions) *
                       tensor_vsize_index_linearization_helper<Index, NumIndices, n - 1, RowMajor>::run(indices,
                                                                                                        dimensions);
            }
        };
    ...
    namespace internal {
        ...
        template<std::ptrdiff_t n, typename std::ptrdiff_t... Indices> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE

        std::ptrdiff_t array_get(const Sizes<Indices...> &) {
            return get<n, internal::numeric_list<std::size_t, Indices...> >::value;
        }

        template<std::ptrdiff_t n> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
    ...
    EIGEN_DEVICE_FUNC bool dimensions_match(Dims1 &dims1, Dims2 &dims2) {
        return internal::sizes_match_below_dim<Dims1, Dims2, internal::array_size<Dims1>::value, internal::array_size<Dims2>::value>::run(
                dims1, dims2);
    }
