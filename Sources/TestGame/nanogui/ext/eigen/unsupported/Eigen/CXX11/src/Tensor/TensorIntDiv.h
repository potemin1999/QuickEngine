  ...
  */
    ...
    namespace internal {
        ...
        namespace {
            ...
            uint64_t muluh(const uint64_t a, const T b) {
#if defined(__CUDA_ARCH__)
            ...
            struct DividerHelper {
                ...
                uint32_t computeMultiplier(const int log_div, const T divider) {
            ...
            struct DividerHelper<64, T> {
                ...
                uint64_t computeMultiplier(const int log_div, const T divider) {
#if defined(__SIZEOF_INT128__) && !defined(__CUDA_ARCH__) && !defined(__SYCL_DEVICE_ONLY__)
                    return static_cast<uint64_t>(
                            (static_cast<__uint128_t>(1) << (64 + log_div)) / static_cast<__uint128_t>(divider) -
        ...
        public:
            ...
            TensorIntDivisor(const T divider) {
                ...
                eigen_assert(static_cast<typename UnsignedTraits<T>::type>(divider) <
            ...
            T divide(const T numerator) const {
                eigen_assert(static_cast<typename UnsignedTraits<T>::type>(numerator) <
                             NumTraits<UnsignedType>::highest() / 2);
