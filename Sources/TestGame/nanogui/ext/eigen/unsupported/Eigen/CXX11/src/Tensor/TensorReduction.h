...
template<typename Op, typename Dims, typename XprType, template<class> class MakePointer_>
struct nested<TensorReductionOp < Op, Dims, XprType, MakePointer_>, 1, typename eval<
...
struct GenericDimReducer {
    ...
    void reduce(const Self &self, typename Self::Index firstIndex, Op &reducer, typename Self::CoeffReturnType *accum) {
...
struct GenericDimReducer<-1, Self, Op> {
    ...
    void reduce(const Self &self, typename Self::Index index, Op &reducer, typename Self::CoeffReturnType *accum) {
...
struct InnerMostDimPreserver {
    ...
    void reduce(const Self &, typename Self::Index, Op &, typename Self::PacketReturnType *) {
...
struct InnerReducer {
    ...
    EIGEN_DEVICE_FUNC static bool
    run(const Self &, Op &, const Device &, typename Self::CoeffReturnType *, typename Self::Index,
        typename Self::Index) {
...
public:
    ...
    TensorReductionOp(const XprType &expr, const Dims &dims, const Op &reducer) : m_expr(expr), m_dims(dims),
                                                                                  m_reducer(reducer) {}
...
struct TensorEvaluator<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device> {
    ...
    typedef typename internal::conditional<NumOutputDims == 0, Sizes<>, DSizes < Index, NumOutputDims> >
    ::type Dimensions;
    ...
    bool evalSubExprsIfNeeded(typename MakePointer_<CoeffReturnType>::Type data) {
        ...
        if ((RunningFullReduction && RunningOnSycl) || (RunningFullReduction &&
            ...
            if (!data) {
                data = static_cast<CoeffReturnType *>(m_device.allocate(
                        sizeof(CoeffReturnType) * num_coeffs_to_preserve));
                m_result = data;
            }
            Op reducer(m_reducer);
            internal::InnerReducer<Self, Op, Device>::run(*this, reducer, m_device, data, num_values_to_reduce,
    ...
    CoeffReturnType coeff(Index index) const {
        ...
        if (ReducingInnerMostDims || RunningFullReduction) {
            const Index num_values_to_reduce =
                    (static_cast<int>(Layout) == static_cast<int>(ColMajor)) ? m_preservedStrides[0]
                                                                             : m_preservedStrides[NumPreservedStrides -
                                                                                                  1];
            return internal::InnerMostDimReducer<Self, Op>::reduce(*this, firstInput(index),
                                                                   num_values_to_reduce, reducer);
        } else {
            typename Self::CoeffReturnType accum = reducer.initialize();
            internal::GenericDimReducer<NumReducedDims - 1, Self, Op>::reduce(*this, firstInput(index), reducer,
                                                                              &accum);
            return reducer.finalize(accum);
        }
    }
    ...
    PacketReturnType packet(Index index) const {
        ...
        if (ReducingInnerMostDims) {
            ...
            for (Index i = 0; i < PacketSize; ++i) {
                Op reducer(m_reducer);
                values[i] = internal::InnerMostDimReducer<Self, Op>::reduce(*this,
                                                                            firstIndex + i * num_values_to_reduce,
                                                                            num_values_to_reduce, reducer);
            }
            ...
            if (((firstIndex % m_dimensions[innermost_dim]) + PacketSize - 1) < m_dimensions[innermost_dim]) {
                ...
                internal::InnerMostDimPreserver<NumReducedDims - 1, Self, Op>::reduce(*this, firstIndex, reducer,
                                                                                      &accum);
    ...
    EIGEN_DEVICE_FUNC typename MakePointer_<Scalar>::Type

    data() const { return m_result; }
...
private:
    template<int, typename, typename> friend
    struct internal::GenericDimReducer;
    template<typename, typename, bool> friend
    struct internal::InnerMostDimReducer;
    template<int, typename, typename, bool> friend
    struct internal::InnerMostDimPreserver;
    template<typename S, typename O, typename D, bool V> friend
    ...
    template<typename S, typename O, typename D> friend
