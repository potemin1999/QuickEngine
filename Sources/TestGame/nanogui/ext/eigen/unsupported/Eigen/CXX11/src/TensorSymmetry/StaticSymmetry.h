...
namespace Eigen {
    ...
    tensor_static_symgroup_index_permute(std::array <Index, N> idx, internal::numeric_list<int, ii...>,
                                         internal::numeric_list<int, jj...>) {
        return {{idx[ii]..., idx[jj]...}};
    }

    template<typename Index, int... ii>
    static inline std::vector <Index>
    tensor_static_symgroup_index_permute(std::vector <Index> idx, internal::numeric_list<int, ii...>) {
    ...
    struct tensor_static_symgroup_do_apply<internal::type_list<first, next...>> {
        ...
        static inline RV run(const std::array <Index, NumIndices> &idx, RV initial, Args &&... args) {
            ...
            typedef typename internal::gen_numeric_list<int,
                    NumIndices - SGNumIndices, SGNumIndices>::type remaining_indices;
        ...
        static inline RV run(const std::vector <Index> &idx, RV initial, Args &&... args) {
            eigen_assert(idx.size() >= SGNumIndices &&
                         "Can only apply symmetry group to objects that have at least the required amount of indices.");
            initial = Op::run(tensor_static_symgroup_index_permute(idx, typename first::indices()), first::flags,
...
public:
    ...
    operator()(Tensor_ &tensor, typename Tensor_::Index firstIndex, IndexTypes... otherIndices) const {
        ...
        return operator()(tensor, std::array < typename Tensor_::Index,
                          Tensor_::NumIndices > {{firstIndex, otherIndices...}});
    }
