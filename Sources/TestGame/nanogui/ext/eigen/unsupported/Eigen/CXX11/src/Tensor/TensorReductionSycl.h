...
namespace Eigen {
    namespace internal {
        ...
        struct syclGenericBufferReducer {
            ...
            static void
            run(OP op, BufferTOut &bufOut, BufferTIn &bufI, const Eigen::SyclDevice &dev, size_t length, size_t local) {
                do {
                    auto f = [length, local, op, &bufOut, &bufI](cl::sycl::handler &h) mutable {
                        ...
                        h.parallel_for(r,
                                       TensorSycl::internal::GenericKernelReducer<CoeffReturnType, OP, OutputAccessor, InputAccessor, LocalAccessor>(
                                               op, aOut, aI, scratch, length, local));
                    };
        ...
        struct syclGenericBufferReducer<Eigen::internal::MeanReducer<CoeffReturnType>, CoeffReturnType> {
            template<typename BufferTOut, typename BufferTIn>
            static void run(Eigen::internal::MeanReducer<CoeffReturnType>, BufferTOut &bufOut, BufferTIn &bufI,
                            const Eigen::SyclDevice &dev, size_t length, size_t local) {
                syclGenericBufferReducer<Eigen::internal::SumReducer<CoeffReturnType>, CoeffReturnType>::run(
                        Eigen::internal::SumReducer<CoeffReturnType>(),
...
// a leafNode.
        ...
        struct FullReducer<Self, Op, const Eigen::SyclDevice, Vectorizable> {
            ...
            static void run(const Self &self, Op &reducer, const Eigen::SyclDevice &dev, CoeffReturnType *output) {
                ...
                size_t rng =
                        inputSize / red_factor; // the total number of thread initially is half the size of the input
                size_t remaining = inputSize % red_factor;
                if (rng == 0) {
                    red_factor = 1;
                };
                size_t tileSize =
                        dev.sycl_queue().get_device().template get_info<cl::sycl::info::device::max_work_group_size>() /
                        2;
                ...
                dev.sycl_queue().submit([&](cl::sycl::handler &cgh) {
                    ...
                    auto tmp_global_accessor = temp_global_buffer.template get_access<cl::sycl::access::mode::read_write, cl::sycl::access::target::global_buffer>(
                            cgh);
                    ...
                    cgh.parallel_for(cl::sycl::nd_range<1>(cl::sycl::range<1>(GRange), cl::sycl::range<1>(outTileSize)),
                                     TensorSycl::internal::FullReductionKernelFunctor<CoeffReturnType, OutAccessor, HostExpr, FunctorExpr, Op, Dims, size_t, TupleType>
                                             (tmp_global_accessor, rng, remaining, red_factor, reducer, self.xprDims(),
                ...
                syclGenericBufferReducer<Op, CoeffReturnType>::run(reducer, out_buffer, temp_global_buffer, dev, GRange,
                                                                   outTileSize);
            }
        ...
        struct InnerReducer<Self, Op, const Eigen::SyclDevice> {
            ...
            static bool run(const Self &self, Op &reducer, const Eigen::SyclDevice &dev, CoeffReturnType *output,
                            typename Self::Index num_values_to_reduce, typename Self::Index num_coeffs_to_preserve) {
                typedef const typename Self::ChildType HostExpr; /// this is the child of reduction
                typedef Eigen::TensorSycl::internal::FunctorExtractor<TensorEvaluator<HostExpr, const Eigen::SyclDevice> > FunctorExpr;
                FunctorExpr functors = TensorSycl::internal::extractFunctors(self.impl());
                ...
                dev.sycl_queue().submit([&](cl::sycl::handler &cgh) {
                    ...
                    auto output_accessor = dev.template get_sycl_accessor<cl::sycl::access::mode::discard_write>(cgh,
                                                                                                                 output);
                    ...
                    cgh.parallel_for(cl::sycl::nd_range<1>(cl::sycl::range<1>(GRange), cl::sycl::range<1>(tileSize)),
                                     TensorSycl::internal::ReductionFunctor<HostExpr, FunctorExpr, Tuple_of_Acc, Dims, Op, typename Self::Index>
                                             (output_accessor, functors, tuple_of_accessors, self.xprDims(), reducer,
                                              range, red_size));
