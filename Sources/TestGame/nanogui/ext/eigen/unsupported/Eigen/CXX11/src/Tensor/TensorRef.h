...
namespace Eigen {
    ...
    namespace internal {
        ...
        public:
            ...
            EIGEN_DEVICE_FUNC virtual const Dimensions
            &

            dimensions() const = 0;

            EIGEN_DEVICE_FUNC virtual const Scalar
            *
            ...
            coeff(DenseIndex
            index) const = 0;
            EIGEN_DEVICE_FUNC virtual Scalar
            &
            coeffRef(DenseIndex
            index) = 0;
...
/** \class TensorRef
  ...
  */
    ...
    public:
        ...
        EIGEN_STRONG_INLINE TensorRef(const Expression &expr) : m_evaluator(
        ...
        operator=(const Expression &expr) {
            unrefEvaluator();
            m_evaluator = new internal::TensorLazyEvaluator<Dimensions, Expression, DefaultDevice>(expr,
                                                                                                   DefaultDevice());
            m_evaluator->incrRefCount();
            return *this;
        }
        ...
        Index rank() const { return m_evaluator->dimensions().size(); }

        EIGEN_DEVICE_FUNC
                EIGEN_STRONG_INLINE

        Index dimension(Index n) const { return m_evaluator->dimensions()[n]; }
        ...
        Index size() const { return m_evaluator->dimensions().TotalSize(); }
        ...
        Scalar &coeffRef(Index i0, Index i1) {
            array<Index, 2> indices;
            indices[0] = i0;
            indices[1] = i1;
            return coeffRef(indices);
        ...
        Scalar &coeffRef(Index i0, Index i1, Index i2) {
            array<Index, 3> indices;
            indices[0] = i0;
            indices[1] = i1;
            indices[2] = i2;
        ...
        Scalar &coeffRef(Index i0, Index i1, Index i2, Index i3, Index i4) {
            array<Index, 5> indices;
            indices[0] = i0;
            indices[1] = i1;
            indices[2] = i2;
...
// evaluator for lvalues
    ...
    struct TensorEvaluator<TensorRef<Derived>, Device> : public TensorEvaluator<const TensorRef<Derived>, Device> {
        ...
        TensorEvaluator(TensorRef<Derived> &m, const Device &d) : Base(m, d) {}
