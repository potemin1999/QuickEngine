...
// Pure fixed-size storage
    ...
    public:
        ...
        const FixedDimensions &dimensions() const { return m_dimensions; }
...
// pure dynamic
    ...
    typedef TensorStorage<T, DSizes < IndexType, NumIndices_>, Options_>
    Self;
...
m_dimensions(dimensions) { EIGEN_INTERNAL_TENSOR_STORAGE_CTOR_PLUGIN }
...
.m_dimensions)
{
internal::smart_copy(other
.m_data, other.m_data+
internal::array_prod(other
.m_dimensions), m_data);
}
EIGEN_DEVICE_FUNC Self
&
...
EIGEN_DEVICE_FUNC ~TensorStorage() {
    internal::conditional_aligned_delete_auto<T, (Options_ & DontAlign) == 0>(m_data, internal::array_prod(
            m_dimensions));
}
