...
template<typename PaddingDimensions, typename XprType>
struct nested<TensorPaddingOp < PaddingDimensions, XprType>, 1, typename eval<
...
public:
    ...
    padding() const { return m_padding_dims; }
...
struct TensorEvaluator<const TensorPaddingOp<PaddingDimensions, ArgType>, Device> {
    ...
    *
    ...
    const PaddingDimensions &padding() const { return m_padding; }

    /// used by sycl
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE

    const Scalar &padding_value() const { return m_paddingValue; }
