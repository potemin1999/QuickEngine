...
struct TensorEvaluator<const TensorEvalToOp<ArgType, MakePointer_>, Device> {
    ...
    dimensions() const { return m_impl.dimensions(); }
    ...
    void evalPacket(Index i) {
        internal::pstoret<CoeffReturnType, PacketReturnType, Aligned>(m_buffer + i,
                                                                      m_impl.template packet<TensorEvaluator<ArgType, Device>::IsAligned
                                                                                             ? Aligned : Unaligned>(i));
    }
    ...
    EIGEN_DEVICE_FUNC DevicePointer
