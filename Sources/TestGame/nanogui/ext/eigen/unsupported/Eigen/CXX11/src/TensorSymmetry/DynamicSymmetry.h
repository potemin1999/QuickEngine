...
namespace Eigen {
    ...
    class DynamicSGroup {
    public:
        inline explicit DynamicSGroup() : m_numIndices(1), m_elements(), m_generators(),
                                          m_globalFlags(0) { m_elements.push_back(ge(Generator(0, 0, 0))); }

        inline DynamicSGroup(const DynamicSGroup &o) : m_numIndices(o.m_numIndices), m_elements(o.m_elements),
                                                       m_generators(o.m_generators), m_globalFlags(o.m_globalFlags) {}

        inline DynamicSGroup(DynamicSGroup &&o) : m_numIndices(o.m_numIndices), m_elements(),
                                                  m_generators(o.m_generators), m_globalFlags(o.m_globalFlags) {
        ...
        inline RV apply(const std::array <Index, N> &idx, RV initial, Args &&... args) const {
            ...
            for (std::size_t i = 0; i < size(); i++)
                initial = Op::run(h_permute(i, idx, typename internal::gen_numeric_list<int, N>::type()),
                                  m_elements[i].flags, initial, std::forward<Args>(args)...);
            return initial;
        }
