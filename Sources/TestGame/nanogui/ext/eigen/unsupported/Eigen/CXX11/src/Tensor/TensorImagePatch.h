...
template<DenseIndex Rows, DenseIndex Cols, typename XprType>
struct nested<TensorImagePatchOp < Rows, Cols, XprType>, 1, typename eval<
...
public:
    ...
    TensorImagePatchOp(const XprType &expr, DenseIndex patch_rows, DenseIndex patch_cols,
...
struct TensorEvaluator<const TensorImagePatchOp<Rows, Cols, ArgType>, Device> {
        ...
        if (op.padding_explicit()) {
            ...
            switch (op.padding_type()) {
                case PADDING_VALID:
                    ...
                    m_rowPaddingTop = numext::maxi<Index>(0, ((m_outputRows - 1) * m_row_strides + m_patch_rows_eff -
                                                              m_input_rows_eff) / 2);
                    m_colPaddingLeft = numext::maxi<Index>(0, ((m_outputCols - 1) * m_col_strides + m_patch_cols_eff -
                                                               m_input_cols_eff) / 2);
                    break;
                case PADDING_SAME:
    ...
    CoeffReturnType coeff(Index index) const {
        ...
        const Index patch2DIndex = (NumDims == 4) ? patchIndex : (index - otherIndex * m_otherStride) /
                                                                 m_fastPatchStride;
    ...
    PacketReturnType packet(Index index) const {
        ...
        const Index patch2DIndex = (NumDims == 4) ? patchIndex : (indices[0] - otherIndex * m_otherStride) /
                                                                 m_fastPatchStride;
