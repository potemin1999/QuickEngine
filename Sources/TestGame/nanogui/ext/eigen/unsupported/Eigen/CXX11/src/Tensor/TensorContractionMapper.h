...
/// is scalar * for CoeffLoader.
        ...
        struct CoeffLoader {
            ...
            void offsetBuffer(typename Tensor::Index) {
                eigen_assert(false && "unsupported");
            }

            EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE

            typename Tensor::Scalar coeff(typename Tensor::Index index) const { return m_tensor.coeff(index); }
        ...
        struct CoeffLoader<Tensor, true, MakePointer_> {
            ...
            CoeffLoader(const Tensor &tensor) : m_data(tensor.data()) {}

            EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE

            void offsetBuffer(typename Tensor::Index offset) {
                m_data += offset;
            }

            EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE

            typename Tensor::Scalar coeff(typename Tensor::Index index) const { return loadConstant(m_data + index); }
        ...
        public:
            ...
            Index computeIndex(Index row, Index col) const {
            ...
            IndexPair <Index> computeIndexPair(Index row, Index col, const Index distance) const {
                const bool left = (side == Lhs);
            ...
            Index stride() const {
                return ((side == Lhs) && inner_dim_contiguous && array_size<contract_t>::value > 0)
                       ? m_contract_strides[0] : 1;
            }
        ...
        public:
            ...
            Packet loadHalfPacket(Index i, Index j) const {
        ...
        public:
            ...
            enum {
                ...
                // TODO: we should also enable direct offsets for the Rhs case.
                        UseDirectOffsets = ParentMapper::DirectOffsets && (side == Lhs) && inner_dim_contiguous &&
                                           (array_size<contract_t>::value > 0)
            };

            EIGEN_DEVICE_FUNC
            TensorContractionSubMapper(const ParentMapper &base_mapper, Index vert_offset, Index horiz_offset)
            ...
            PacketT loadPacket(Index i, Index j) const {
                if (UseDirectOffsets) {
        ...
        public:
            ...
            SubMapper getSubMapper(Index i, Index j) const {
