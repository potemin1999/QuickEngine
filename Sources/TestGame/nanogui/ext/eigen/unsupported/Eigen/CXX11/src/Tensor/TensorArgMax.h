...
public:
    ...
    TensorIndexTupleOp(const XprType &expr)
            : m_xpr(expr) {}
...
public:
    ...
    TensorTupleReducerOp(const XprType &expr,
    ...
    reduce_op() const { return m_reduce_op; }

    EIGEN_DEVICE_FUNC
    const Dims
    &

    reduce_dims() const { return m_reduce_dims; }
    ...
    *
    ...
    costPerCoeff(bool vectorized) const {
        const double compute_cost = 1.0 +
                                    (m_return_dim < 0 ? 0.0 : (TensorOpCost::ModCost<Index>() +
