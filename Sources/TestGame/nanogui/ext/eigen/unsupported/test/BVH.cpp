...
namespace Eigen {
    ...
    AlignedBox<Scalar, Dim> bounding_box(const Matrix<Scalar, Dim, 1> &v) { return AlignedBox<Scalar, Dim>(v); }
...
struct TreeTest {
    typedef Matrix<double, Dim, 1> VectorType;
    typedef std::vector<VectorType, aligned_allocator < VectorType> >
    VectorTypeList;
    typedef Ball<Dim> BallType;
    typedef std::vector<BallType, aligned_allocator < BallType> >
    BallTypeList;
    typedef AlignedBox<double, Dim> BoxType;
