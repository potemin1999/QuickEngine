                    ...
                    for (int d = 0; d < 2; ++d) {
                        for (int b = 0; b < 7; ++b) {
                            ...
                            if (entire_image_patch(d, r, c, patchId, b) != expected) {
                                std::cout << "Mismatch detected at index i=" << i << " j=" << j << " r=" << r << " c="
                                          << c << " d=" << d << " b=" << b << std::endl;
                            }
...
void test_patch_padding_valid() {
    ...
    Tensor<float, 5, RowMajor> result_row_major = tensor_row_major.extract_image_patches(ksize, ksize, stride, stride,
                                                                                         1, 1, PADDING_VALID);
