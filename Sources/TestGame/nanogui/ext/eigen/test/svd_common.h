...
void svd_min_norm(const MatrixType &m, unsigned int computationOptions) {
    ...
    Matrix<Scalar, Dynamic, 1> tmp = qr.matrixQR().topLeftCorner(rank,
                                                                 rank).template triangularView<Upper>().adjoint().solve(
            rhs2);
    ...
    SVD_FOR_MIN_NORM(MatrixType3)
    svd3(m3, computationOptions);
...
#pragma warning disable 111
    ...
    if (MatrixType::ColsAtCompileTime == Dynamic) {
        ...
        VERIFY_IS_APPROX(m, svd.matrixU().leftCols(diagSize) * svd.singularValues().asDiagonal() *
                            svd.matrixV().leftCols(diagSize).adjoint());
    }
}
...
template<typename T> EIGEN_DONT_INLINE T
sub(T
a,
T b
) {
...
void svd_preallocate() {
    ...
    VERIFY_RAISES_ASSERT(VectorXf
    tmp(10);)
    SVD_DEFAULT(MatrixXf)
    svd;
    ...
    SVD_DEFAULT(MatrixXf)
    svd3(3, 3, ComputeFullU | ComputeFullV);
    internal::set_is_malloc_allowed(false);
    svd2.compute(m);
    internal::set_is_malloc_allowed(true);
    VERIFY_IS_APPROX(svd2.singularValues(), v);
