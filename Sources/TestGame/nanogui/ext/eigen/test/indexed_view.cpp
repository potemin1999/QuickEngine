...
void check_indexed_view() {
    ...
    VERIFY(is_same_seq_type(seq(std::integral_constant<int, 1>(), std::integral_constant<int, 5>()),
                            seq(fix<1>, fix<5>)));
#else
    ...
    VERIFY_IS_APPROX(A(seq(n - 1, 2, -2), seqN(n - 1 - 6, 3, -1)),
                     A(seq(last, 2, fix<-2>), seqN(last - 6, 3, fix<-1>)));
