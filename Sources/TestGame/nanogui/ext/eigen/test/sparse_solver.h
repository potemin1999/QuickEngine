...
void check_sparse_solving(Solver &solver, const typename Solver::MatrixType &A, const Rhs &b, const DenseMat &dA,
    ...
    DenseRhs refX = dA.householderQr().solve(db);
    {
        ...
        x.setZero();
        // test with Map
        MappedSparseMatrix<Scalar, Mat::Options, StorageIndex> Am(A.rows(), A.cols(), A.nonZeros(),
...
template<typename Solver, typename DenseMat>
int
generate_sparse_spd_problem(Solver &, typename Solver::MatrixType &A, typename Solver::MatrixType &halfA, DenseMat &dA,
                            int maxSize = 300) {
    typedef typename Solver::MatrixType Mat;
    typedef typename Mat::Scalar Scalar;
