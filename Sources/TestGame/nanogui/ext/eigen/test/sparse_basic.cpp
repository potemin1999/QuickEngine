...
void sparse_basic(const SparseMatrixType &ref) {
    ...
    Scalar s1 = internal::random<Scalar>();
    {
        ...
        for (std::size_t i = 0; i < zeroCoords.size(); ++i) {
            ...
            if (internal::is_same<SparseMatrixType, SparseMatrix<Scalar, Flags> >::value) VERIFY_RAISES_ASSERT(
                    m.coeffRef(zeroCoords[i].x(), zeroCoords[i].y()) = 5);
        }
    ...
    for (int mode = 0; mode < 4; ++mode) {
        ...
        m1.setZero();
        SparseMatrixType m2(rows, cols);
        VectorXi r(VectorXi::Constant(m2.outerSize(), ((mode % 2) == 0) ? int(m2.innerSize()) : std::max<int>(1,
                                                                                                              int(m2.innerSize()) /
                                                                                                              8)));
        m2.reserve(r);
...
//     VERIFY_IS_APPROX(m3.cwise()/refM4, refM3.cwise()/refM4);
    ...
    // test Map
    {
        ...
        {
            MappedSparseMatrix<Scalar, SparseMatrixType::Options, StorageIndex> mapMat2(m2.rows(), m2.cols(),
                                                                                        m2.nonZeros(),
                                                                                        m2.outerIndexPtr(),
                                                                                        m2.innerIndexPtr(),
                                                                                        m2.valuePtr(),
                                                                                        m2.innerNonZeroPtr());
            MappedSparseMatrix<Scalar, SparseMatrixType::Options, StorageIndex> mapMat3(m3.rows(), m3.cols(),
                                                                                        m3.nonZeros(),
                                                                                        m3.outerIndexPtr(),
                                                                                        m3.innerIndexPtr(),
                                                                                        m3.valuePtr(),
