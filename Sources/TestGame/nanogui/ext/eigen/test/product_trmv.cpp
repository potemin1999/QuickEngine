...
void trmv(const MatrixType &m) {
    ...
    m3 = m1.template triangularView<Eigen::Lower>();
    VERIFY(((s1 * m3).conjugate() * v1).isApprox((s1 * m1).conjugate().template triangularView<Eigen::Lower>() * v1,
                                                 largerEps));
    m3 = m1.template triangularView<Eigen::Upper>();
    VERIFY((m3.conjugate() * v1.conjugate()).isApprox(
    ...
    m3 = m1.template triangularView<Eigen::Lower>();
    VERIFY((m3.adjoint() * (s1 * v1.conjugate())).isApprox(
            m1.adjoint().template triangularView<Eigen::Upper>() * (s1 * v1.conjugate()), largerEps));
    m3 = m1.template triangularView<Eigen::UnitUpper>();
    ...
    VERIFY((v1.adjoint() * m3.adjoint()).isApprox(v1.adjoint() * m1.template triangularView<Eigen::Lower>().adjoint(),
                                                  largerEps));
