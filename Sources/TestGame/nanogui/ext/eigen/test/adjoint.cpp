...
struct adjoint_specific<false> {
    ...
    static void run(const Vec &v1, const Vec &v2, Vec &v3, const Mat &square, Scalar s1, Scalar s2) {
        ...
        RealScalar ref = NumTraits<Scalar>::IsInteger ? RealScalar(0) : (std::max)((s1 * v1 + s2 * v2).norm(),
                                                                                   v3.norm());
        VERIFY(test_isApproxWithRef((s1 * v1 + s2 * v2).dot(v3),
        ...
        // check compatibility of dot and adjoint
        ref = NumTraits<Scalar>::IsInteger ? 0 : (std::max)((std::max)(v1.norm(), v2.norm()),
                                                            (std::max)((square * v2).norm(),
                                                                       (square.adjoint() * v1).norm()));
...
void test_adjoint() {
    for (int i = 0; i < g_repeat; i++) {
        ...
        CALL_SUBTEST_5(adjoint(MatrixXi(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                                        internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
        CALL_SUBTEST_6(adjoint(MatrixXf(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                                        internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
