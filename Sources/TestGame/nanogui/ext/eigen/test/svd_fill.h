...
void svd_fill_random(MatrixType &m, int Option = 0) {
    ...
    bool unit_uv = internal::random<int>(0, 10) < (dup ? 7
                                                       : 3); // if we duplicate some diagonal entries, then increase the chance to preserve them using unitary U and V factors
    ...
    samples << 0, four_denorms<RealScalar>(),
            -RealScalar(1) / NumTraits<RealScalar>::highest(), RealScalar(1) /
                                                               NumTraits<RealScalar>::highest(), (std::numeric_limits<RealScalar>::min)(), pow(
            (std::numeric_limits<RealScalar>::min)(), 0.8);
                ...
                if (!(dup && unit_uv)) {
                    ...
                    for (Index k = 0; k < n; ++k) {
                        ...
                        if (NumTraits<Scalar>::IsComplex)
                            *(&numext::real_ref(m(j, i)) + 1) = *(&numext::real_ref(m(i, j)) + 1) = samples.real()(
