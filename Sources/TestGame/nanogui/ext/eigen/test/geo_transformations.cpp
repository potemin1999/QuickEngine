...
void non_projective_only() {
  ...
  */
    ...
    t0.setIdentity();
    t0.translate(v0);
    VERIFY_IS_APPROX((t0 * v1).template head<3>(), Translation3(v0)
            *v1);

    // AlignedScaling * vector
    t0.setIdentity();
    t0.scale(v0);
    VERIFY_IS_APPROX((t0 * v1).template head<3>(), AlignedScaling3(v0)
            *v1);
}
...
void transformations() {
    ...
    */
    ...
    VERIFY_IS_APPROX(q1 * v1, Quaternionx(aa)
            *v1);
...
void transform_alignment() {
    ...
    *p3 = *p1;
    ...
    if (internal::packet_traits<Scalar>::Vectorizable) VERIFY_RAISES_ASSERT(
