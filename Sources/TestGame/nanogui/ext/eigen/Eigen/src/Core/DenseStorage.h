...
// matrix with dynamic width and fixed height (so that matrix has dynamic size).
    ...
    public:
        ...
        EIGEN_DEVICE_FUNC DenseStorage(Index size, Index rows, Index cols) : m_data(
                internal::conditional_aligned_new_auto<T, (_Options & DontAlign) == 0>(size)), m_cols(cols) {
            EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN({})
...
// matrix with dynamic height and fixed width (so that matrix has dynamic size).
    ...
    public:
        ...
        EIGEN_DEVICE_FUNC DenseStorage(Index size, Index rows, Index cols) : m_data(
                internal::conditional_aligned_new_auto<T, (_Options & DontAlign) == 0>(size)), m_rows(rows) {
            EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN({})
