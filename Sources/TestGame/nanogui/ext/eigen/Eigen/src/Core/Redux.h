            ...
            static Scalar run(const Derived &mat, const Func &func) {
                ...
                enum {
                    alignment0 = (bool(Derived::Flags & DirectAccessBit) &&
                                  bool(packet_traits<Scalar>::AlignedOnScalar)) ? int(packetAlignment) : int(Unaligned),
                ...
                if (alignedSize) {
                    ...
                    if (alignedSize > packetSize) // we have at least two packets to partly unroll the loop
                    {
                        ...
                        for (Index index = alignedStart + 2 * packetSize;
                             index < alignedEnd2; index += 2 * packetSize) {
                            packet_res0 = func.packetOp(packet_res0,
                                                        mat.template packet<alignment, PacketScalar>(index));
                            packet_res1 = func.packetOp(packet_res1, mat.template packet<alignment, PacketScalar>(
                                    index + packetSize));
                        }
...
// NOTE: for SliceVectorizedTraversal we simply bypass unrolling
        ...
        struct redux_impl<Func, Derived, LinearVectorizedTraversal, CompleteUnrolling> {
            ...
            Scalar run(const Derived &mat, const Func &func) {
                ...
                if (VectorizedSize > 0) {
                    Scalar
                    res = func.predux(redux_vec_unroller<Func, Derived, 0, Size / PacketSize>::run(mat, func));
                    if (VectorizedSize != Size)
                        res = func(res,
                                   redux_novec_unroller<Func, Derived, VectorizedSize, Size - VectorizedSize>::run(mat,
                                                                                                                   func));
                    return res;
                } else {
...
// evaluator adaptor
        ...
        public:
            ...
            PacketType packetByOuterInner(Index outer, Index inner) const {
                return m_evaluator.template packet<LoadMode, PacketType>(IsRowMajor ? outer : inner,
