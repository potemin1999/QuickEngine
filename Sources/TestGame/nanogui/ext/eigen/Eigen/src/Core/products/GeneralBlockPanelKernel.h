 ...
 * \sa setCpuCacheSizes */
    ...
    l3 = 512*1024;
                ...
                if (k > max_kc) {
                    ...
                    k = (k % max_kc) == 0 ? max_kc
                                          : max_kc -
                                            k_peeling * ((max_kc - 1 - (k % max_kc)) / (k_peeling * (k / max_kc + 1)));
...
::operator()(const DataMapper &res, const LhsScalar *blockA, const RhsScalar *blockB,
    ...
    const Index peeled_mc2 =
            mr >= 2 * Traits::LhsProgress ? peeled_mc3 + ((rows - peeled_mc3) / (2 * LhsProgress)) * (2 * LhsProgress)
                                          : 0;
...
//     const Index depth2     = depth & ~1;
    ...
    if (mr >= 3 * Traits::LhsProgress) {
        ...
        const Index actual_panel_rows = (3 * LhsProgress) * std::max<Index>(1, ((l1 - sizeof(ResScalar) * mr * nr -
                                                                                 depth * nr * sizeof(RhsScalar)) /
                                                                                (depth * sizeof(LhsScalar) * 3 *
                                                                                 LhsProgress)));
...
#undef EIGEN_GEBGP_ONESTEP
    ...
    if (mr >= 2 * Traits::LhsProgress) {
        ...
        // suggests we should be using: either because our known l1 cache size is inaccurate (e.g. on Android, we can only guess),
        // or because we are testing specific blocking sizes.
        Index actual_panel_rows = (2 * LhsProgress) * std::max<Index>(1, ((l1 - sizeof(ResScalar) * mr * nr -
                                                                           depth * nr * sizeof(RhsScalar)) /
                                                                          (depth * sizeof(LhsScalar) * 2 *
                                                                           LhsProgress)));
