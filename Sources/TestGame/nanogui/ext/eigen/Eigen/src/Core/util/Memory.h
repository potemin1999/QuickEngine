...
/** \internal Constructs the elements of an array.
  ...
  */
        template<typename T> EIGEN_DEVICE_FUNC inline T
        *
        construct_elements_of_array(T
        *ptr,
        std::size_t size
        ) {
        std::size_t i;
...
template<typename T, bool Align> EIGEN_DEVICE_FUNC inline T
*
conditional_aligned_new(std::size_t
size)
{
check_size_for_overflow <T>(size);
...
template<typename T, bool Align> EIGEN_DEVICE_FUNC inline T
*
conditional_aligned_realloc_new(T
* pts,
...
destruct_elements_of_array(pts
...
EIGEN_TRY
{
construct_elements_of_array(result
...
template<typename T, bool Align> EIGEN_DEVICE_FUNC inline T
*
conditional_aligned_new_auto(std::size_t
size)
{
if (size==0)
...
first_aligned(const Scalar *array, Index size) {
    ...
    if (AlignmentSize <= 1) {
        ...
        Index
        first = (AlignmentSize - (Index((UIntPtr(array) / sizeof(Scalar))) & AlignmentMask)) & AlignmentMask;
