...
/** \eigenvalues_module \ingroup Eigenvalues_Module
  ...
  */
    ...
    MatrixType EigenSolver<MatrixType>::pseudoEigenvalueMatrix() const {
        ...
        for (Index i = 0; i < n; ++i) {
            if (internal::isMuchSmallerThan(numext::imag(m_eivalues.coeff(i)), numext::real(
                    m_eivalues.coeff(i)), precision))
    ...
    typename EigenSolver<MatrixType>::EigenvectorsType EigenSolver<MatrixType>::eigenvectors() const {
        ...
        for (Index j = 0; j < n; ++j) {
            if (internal::isMuchSmallerThan(numext::imag(m_eivalues.coeff(j)), numext::real(
                    m_eivalues.coeff(j)), precision) || j + 1 == n)
            {
                        ...
                        if (m_eivalues.coeff(i).imag() == RealScalar(0)) {
                            ...
                            if (abs(x) > (abs(lastw) + abs(q))) {
                                ...
                                cc = ComplexScalar(-lastra - y * m_matT.coeff(i, n - 1),
                                                   -lastsa - y * m_matT.coeff(i, n)) / ComplexScalar(lastw, q);
