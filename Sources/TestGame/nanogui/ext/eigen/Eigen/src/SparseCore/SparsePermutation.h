...
namespace Eigen {
    ...
    namespace internal {
        ...
        struct permutation_matrix_product<ExpressionType, Side, Transposed, SparseShape> {
            ...
            SparseMatrix<Scalar, int(SrcStorageOrder) == RowMajor ? ColMajor : RowMajor, StorageIndex> >
            ::type ReturnType;
                ...
                if (MoveOuter) {
                    ...
                    for (Index j = 0; j < mat.outerSize(); ++j) {
                        ...
                        sizes[((Side == OnTheLeft) ^ Transposed) ? jp : j] = StorageIndex(
                                mat.innerVector(((Side == OnTheRight) ^ Transposed) ? jp : j).nonZeros());
                    }
                    tmp.reserve(sizes);
                    for (Index j = 0; j < mat.outerSize(); ++j) {
                        Index jp = perm.indices().coeff(j);
...
// whereas it should be correctly handled by traits<Product<> >::PlainObject
        ...
        explicit product_evaluator(const XprType &xpr)
            ...
            generic_product_impl<Lhs, Rhs, PermutationShape, SparseShape, ProductTag>::evalTo(m_result, xpr.lhs(),
                                                                                              xpr.rhs());
        }
    ...
    explicit product_evaluator(const XprType &xpr)
            : m_result(xpr.rows(), xpr.cols()) {
        ::new(static_cast<Base *>(this)) Base(m_result);
        generic_product_impl<Lhs, Rhs, SparseShape, PermutationShape, ProductTag>::evalTo(m_result, xpr.lhs(),
                                                                                          xpr.rhs());
    }
