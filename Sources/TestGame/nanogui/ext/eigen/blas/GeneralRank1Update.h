...
/* Optimized matrix += alpha * uv' */
    ...
    struct general_rank1_update<Scalar, Index, ColMajor, ConjLhs, ConjRhs> {
        static void
        run(Index rows, Index cols, Scalar *mat, Index stride, const Scalar *u, const Scalar *v, Scalar alpha) {
            ...
            for (Index i = 0; i < cols; ++i)
                Map < Matrix < Scalar, Dynamic,
                        1 > > (mat + stride * i, rows) += alpha * cj(v[i]) * ConjRhsType(OtherMap(u, rows));
        }
    };
