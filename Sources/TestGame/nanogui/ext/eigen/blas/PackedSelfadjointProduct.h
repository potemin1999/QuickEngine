 ...
 */
        ...
        static void run(Index size, Scalar *mat, const Scalar *vec, RealScalar alpha) {
            ...
            for (Index i = 0; i < size; ++i) {
                Map < Matrix < Scalar, Dynamic,
                        1 > > (mat, UpLo == Lower ? size - i : (i + 1)) += alpha * cj(vec[i]) * ConjRhsType(
                                OtherMap(vec + (UpLo == Lower ? i : 0), UpLo == Lower ? size - i : (i + 1)));
    ...
    struct selfadjoint_packed_rank1_update<Scalar, Index, RowMajor, UpLo, ConjLhs, ConjRhs> {
        ...
        static void run(Index size, Scalar *mat, const Scalar *vec, RealScalar alpha) {
            selfadjoint_packed_rank1_update<Scalar, Index, ColMajor,
                    UpLo == Lower ? Upper : Lower, ConjRhs, ConjLhs>::run(size, mat, vec, alpha);
        }
    };
