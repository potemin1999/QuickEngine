...
#undef DECL_NPY_API
    ...
    PYBIND11_OBJECT_DEFAULT(dtype, object, detail::npy_api::get().PyArrayDescr_Check_);
        ...
        explicit dtype(const buffer_info &info) {
            dtype
            descr(_dtype_from_pep3118()(PYBIND11_STR_TYPE(info.format)));
    ...
    PYBIND11_OBJECT_CVT(array, buffer, detail::npy_api::get().PyArray_Check_, raw_array)
        ...
        template<typename... Ix>
        size_t offset_at(Ix... index) const {
        ...
        /// May throw if the index would lead to out of bounds access.
        template<typename... Ix>
        size_t index_at(Ix... index) const {
    ...
    protected:
        ...
        template<typename... Ix>
        size_t byte_offset(Ix... index) const {
            check_dimensions(index...);
            return byte_offset_unsafe(index...);
        }

        template<size_t dim = 0, typename... Ix>
        size_t byte_offset_unsafe(size_t i, Ix... index) const {
    ...
    NAMESPACE_BEGIN(detail)
        ...
        struct is_pod_struct {
            enum {
                value = std::is_pod<T>::value && // offsetof only works correctly for POD types
        ...
        template<typename T>
        struct npy_format_descriptor<T, enable_if_t<std::is_integral<T>::value>> {
        private:
        ...
        public:
            enum {
                value = values[detail::log2(sizeof(T)) * 2 + (std::is_unsigned<T>::value ? 1 : 0)]
            };
...
#define PYBIND11_NUMPY_DTYPE_EX(Type, ...) \
    ::pybind11::detail::npy_format_descriptor<Type>::register_dtype \
        ...
        struct vectorize_helper {
            ...
            object run(array_t<Args, array::c_style | array::forcecast> &... args, index_sequence<Index...> index) {
