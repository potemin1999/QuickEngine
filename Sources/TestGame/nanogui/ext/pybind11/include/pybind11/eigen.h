            ...
            bool load(handle src, bool) {
                ...
                if (buf.ndim() == 1) {
                    ...
                    if ((Type::RowsAtCompileTime != Eigen::Dynamic &&
                         buf.shape(0) != (size_t) Type::RowsAtCompileTime) ||
...
// type_caster to handle argument copying/forwarding.
        ...
        public:
            bool load(handle src, bool convert) {
                if (derived_caster.load(src, convert)) {
                    value.reset(new Type(derived_caster.operator Derived &()));
                    return true;
                }
                return false;
            }
            ...
            static handle cast(const Type &src, return_value_policy policy, handle parent) {
                return DerivedCaster::cast(src, policy, parent);
            }
            ...
            operator Type &() {
                if (!value) pybind11_fail("Eigen::Ref<...> value not loaded");
                return *value;
            }
...
// cast them into the python domain by first copying to a regular Eigen::Matrix, then casting that.
        ...
        public:
            [[noreturn]] bool load(handle, bool) {
                pybind11_fail("Unable to load() into specialized EigenBase object");
            }

            static handle cast(const Type &src, return_value_policy policy, handle parent) {
                return MatrixCaster::cast(Matrix(src), policy, parent);
            }
        ...
        PYBIND11_TYPE_CASTER(Type, _<(Type::Flags & Eigen::RowMajorBit) != 0>("scipy.sparse.csr_matrix[",
