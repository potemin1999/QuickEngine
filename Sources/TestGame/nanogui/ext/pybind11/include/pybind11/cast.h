...
// so, copy constructability depends on whether the value_type is copy constructible.
        ...
        struct is_copy_constructible<Container, enable_if_t<
...
/// Generic type caster for objects stored on the heap
        ...
        protected:
            ...
            static auto make_copy_constructor(const T *value) -> decltype(new T(*value), Constructor(nullptr)) {
                return [](const void *arg) -> void * { return new T(*((const T *) arg)); };
            }

            template<typename T = type>
            static auto
            make_move_constructor(const T *value) -> decltype(new T(std::move(*((T *) value))), Constructor(nullptr)) {
...
// Shortcut for calling a caster's `cast_op_type` cast operator for casting a type_caster to a T
        ...
        template<typename type>
        class type_caster<std::reference_wrapper<type>> : public type_caster_base<type> {
        public:
...
#define PYBIND11_TYPE_CASTER(type, py_name) \
    ...
    public: \
        ...
        template<>
        class type_caster<std::nullptr_t> : public type_caster<void_type> {
        };
        ...
        public:
            bool load(handle src, bool) {
                ...
                if (temp) {
                    ...
                    length = length / (ssize_t) sizeof(wchar_t) - 1;
                    ++buffer; // Skip BOM
                }
#endif
                if (!buffer) {
        ...
        template<>
        class type_caster<char> : public type_caster<std::string> {
        public:
        ...
        template<>
        class type_caster<wchar_t> : public type_caster<std::wstring> {
        public:
            bool load(handle src, bool convert) {
                if (src.is_none()) return true;
        ...
        class type_caster<std::tuple<Tuple...>> {
            ...
            static handle cast_impl(const type &src, return_value_policy policy, handle parent, index_sequence<Is...>) {
                std::array<object, size> entries{{
                                                         reinterpret_steal<object>(
                                                                 make_caster<Tuple>::cast(std::get<Is>(src), policy,
                                                                                          parent))...
                                                 }};
...
/// Type caster for holder types like std::shared_ptr, etc.
        ...
        class type_caster_holder : public type_caster_base<type> {
        public:
...
// Specialization for always-supported unique_ptr holders:
        ...
        struct handle_type_name {
            static PYBIND11_DESCR name() { return _<T>(); }
        };

        template<>
        struct handle_type_name<bytes> {
            static PYBIND11_DESCR name() { return _(PYBIND11_BYTES_NAME); }
        };

        template<>
        struct handle_type_name<args> {
            static PYBIND11_DESCR name() { return _("*args"); }
        };

        template<>
        struct handle_type_name<kwargs> {
            static PYBIND11_DESCR name() { return _("**kwargs"); }
        };
