...
struct NestABase {
    int value = -2; TRACKERS(NestABase)
};
...
public:
    ...
    custom_unique_ptr(custom_unique_ptr<T> &&move) : _ptr{move._ptr} {
        move._ptr = nullptr;
        print_move_created(this);
    }

    custom_unique_ptr &operator=(custom_unique_ptr<T> &&move) {
        print_move_assigned(this);
        if (_ptr) destruct_ptr();
        _ptr = move._ptr;
        move._ptr = nullptr;
        return *this;
    }
...
struct TplConstrClass {
    template<typename T>
    TplConstrClass(const T &arg) : str{arg} {}
...
namespace std {
    template<>
    struct hash<TplConstrClass> {
        size_t operator()(const TplConstrClass &t) const { return std::hash<std::string>()(t.str); }
    };
}
...
void init_issues(py::module &m) {
    ...
    struct ElementBase {
        virtual void foo() {} /* Force creation of virtual table */ };
    ...
    py::class_<NestA>(m2, "NestA").def(py::init<>()).def(py::self += int())
            .def("as_base", [](NestA &a) -> NestABase & { return (NestABase &) a; },
...
//      .def("str_ref", &OverrideTest::str_ref)
    ...
    public:
        ...
        SpecialHolderObj(int v, bool make_child = true) : val{v}, ch{make_child ? new SpecialHolderObj(val + 1, false)
                                                                                : nullptr} { print_created(this, val); }
